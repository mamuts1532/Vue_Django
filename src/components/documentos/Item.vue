<template>
    <div>
        ...
        <v-list-item ripple>
            ...
            <v-btn @click="abrirModal" text icon color="grenn lighten-2">
                <v-icon>edit</v-icon>
            </v-btn>
            <v-list-item-content>
            ...
            <v-list-item-action>
                ...
                <v-icon v-if="!expirado(item.expira)" color="green lighten-1">done_outlime</v-icon>
                <v-icon v-else color="red lighten-1">warning</v-icon>
            </v-list-item-action>
        </v-list-item>
            <v-divider :key="`dividir-${item.id}`"></v-divider>
    </div>
...
export default {
    ...
    methods: {
        borrarDocumento(){
            let id = this.item.id;
            try {
                this.$store.commit("mostrarLoading","Cargo Item");
