<template>
    <b-container fluid>
        ...
        centered hide-header-close header-bg-variant="success" ok-only ok-variant="info" ok-title="Salir"> 
        <b-container fluid>
            <b-row>
                <b-col>
                    <b-card title="Buscar">
                ...
                <b-col>
                    <b-card title="Resultados">
                        <b-overlay :show="loading" spinner-variant="primary" rounded="sm">
                            ...
                            emptyFilteredText = "No se encontró ningun registro"
        ...
        <b-row>
            <b-col>
                <b-card title="Detalle" class="mb-2">
                    ...
                    show-empty
                    emptyText="No hay Datos"
                    emptyFilterdText="No se encontró ningún registro"
                    foot-clone
                    >
    ...
    methods: {
        ...
        async buscarProducto(){
            if(this.detalle.producto>0){
                try {
                    this.loading = true
                    const p = await this.apiInv.getProducto(this.detalle.producto)
                    if (p.id!==undefined) {
                        if (p.existencia>0) {
        ...
        async  save(){
            ...
            try {
                ...
                if (f.id===undefined) {
                    ...
                    if (d.id===undefined) {
                        this.msgError(`Falló en Guardar Detalle con: ${d}`);
        ...
        async refresh(){
            try {
                ...
                if (r.detail != undefined) {
                    ...
                    this.encabezado = r;
                    this.encabezado.cliente = await this.api.getCliente(this.encabezado.cliente);
                    this.encabezado.fecha = moment(r.fecha,'YYY-DD-DD').format('DD/MM/YYYY');
