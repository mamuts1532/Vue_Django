      ...
      <v-container>
        ...
        >
            ...
            <template v-slot:item.actions="{ item }">
              <v-icon color="danger" small @click="borrarDetalle(item)">delete</v-icon>
            </template>
...
export default {
  ...
  methods: {
    async iniciar() {
      ...
      // console.log(r);
      this.proveedores = r;
      this.productos = await this.apiInv.getProducto();
      // console.log(this.productos);

      this.editedDetalle = Object.assign({},this.detalle_inicial)
      this.editedEnc = Object.assign({}, this.encabezado_icicial)
      this.detalle = []
    ...
    async save(){
      ...
      if(det.cantidad<=0){
        // alert("Cantidad Erronea - No se aceptan Cero o Negativo")
        this.$swal("Cantidad Erronea","No se aceptan Cero o Negativo","error")
        return false
      }

      if(det.precio<0){
        alert("Precio Erroneo - No se acepta Negativo")
        return false
      }
    ...
    async borrarDetalle(item){
      // if (confirm("Borrar Detalle")) {
        ...
        const result = await this.mensajeSiNo(`${item.producto_descripcion} con id ${item.id}?`,"¿Borrar")
